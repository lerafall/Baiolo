/* Crimson Path — engine.
 *
 * A precision platformer: run, variable jump, wall jump and an 8-way cloak
 * dash. Instant respawns, generous coyote time and jump buffering keep the
 * feel forgiving; the levels do the teaching. Little Red Riding Hood edition.
 */
(() => {
  "use strict";

  const Art = window.CrimsonArt;
  const Levels = window.CrimsonLevels;
  const T = Levels.T;
  const TILE = Art.TILE;
  ...
  const STORAGE_KEY = "baiolo.crimson-path.v1";
  ...
  const els = {
    ...
    storyText: $("storyText"),
    storyContinue: $("storyContinue"),
  };

  const STORY = {
    0: {
      eyebrow: "Once upon a time…",
      text: "Mother packed a basket of warm pies. \"Stay on the path, little one — and mind the wolf.\"",
    },
    4: {
      eyebrow: "Deeper into the woods",
      text: "The trees lean close. Red learns to cling to bark and kick off into the dark thicket.",
    },
    7: {
      eyebrow: "Nearly there",
      text: "Lanterns bloom along Cottage Lane. Grandmother's chimney smoke curls above the dusk.",
    },
    9: {
      eyebrow: "The wolf at the door",
      text: "A shadow stretches behind her. Don't look back — the cottage light is waiting.",
    },
  };
  ...
  function load() {
    try {
      ...
      return {
        ...defaultSave(),
        ...parsed,
        storySeen: { ...defaultSave().storySeen, ...(parsed.storySeen || {}) },
      };
    } catch {
      return defaultSave();
    }
  ...
  function startLevel(index, keepStats) {
    try {
      ...
      if (beat && !keepStats && !save.storySeen[String(index)]) {
        pendingStory = { index, keepStats: !!keepStats, beat };
        hideCards();
        els.hud.hidden = true;
        if (els.storyEyebrow) els.storyEyebrow.textContent = beat.eyebrow;
        if (els.storyText) els.storyText.textContent = beat.text;
        if (els.story) els.story.hidden = false;
        state.mode = "story";
        return;
      }
  ...
  function continueStory() {
    ...
    const { index, keepStats } = pendingStory;
    save.storySeen[String(index)] = true;
    persist();
    pendingStory = null;
    if (els.story) els.story.hidden = true;
    try {
      loadLevel(index, keepStats);
    } catch (err) {
      state.fade = 0;
  ...
  function loadLevel(index, keepStats) {
    ...
    for (const e of level.entities) {
      ...
      switch (e.type) {
        ...
        case "bug":
        case "wolf":
          ents.push({ ...base, type: "wolf", py: e.y * TILE + TILE, dir: -1, dead: 0, walk: 0 });
          break;
        case "wisp":
      ...
      switch (e.type) {
        ...
        case "petal": {
          ...
          if (fox.alive && overlapsFox(e.px, e.py, 30, 30)) {
            ...
            toast("Wildflower found", "A rare bloom for your cloak collection");
            updateHud();
          }
          break;
        }
        ...
        case "check": {
          if (!e.lit && fox.alive && overlapsFox(e.px, e.py - 20, 40, 44)) {
            ...
            toast("Lantern lit", "You'll wake here");
          }
          break;
        }
        case "bug":
        case "wolf": {
          ...
          const aheadX = Math.floor((nx + e.dir * 14) / TILE);
          const footY = Math.floor((e.py + 4) / TILE);
          const wallAhead = isSolid(aheadX, Math.floor((e.py - 10) / TILE));
          const groundAhead = isSolid(aheadX, footY) || tileAt(aheadX, footY) === T.ONEWAY;
          if (wallAhead || !groundAhead) e.dir *= -1;
          else e.px = nx;
          if (fox.alive && overlapsFox(e.px, e.py - 14, 36, 28)) {
            const stomping = fox.vy > 60 && fox.y < e.py - 8;
            const dashing = fox.dashT > 0;
            if (stomping || dashing) {
              e.dead = 0.001;
              if (stomping) fox.vy = -STOMP_VEL;
              fox.dashReady = true;
              fox.squash = 1.25;
              SFX.stomp();
              shake(4);
              addParticles("ember", e.px, e.py - 10, 18, {
                color: "#c8b090",
                speed: 180,
                life: 0.55,
                grav: 280,
  ...
  function winLevel(exitEnt) {
    ...
    const fullBasket = state.seeds >= level.seedTotal && level.seedTotal > 0;
    els.winTitle.textContent = first
      ? "Basket delivered!"
      : improved
        ? "New best time!"
        : "Cottage reached!";
    ...
    els.winStats.innerHTML = `
      <span>🥧 ${state.seeds}/${level.seedTotal}${fullBasket ? " · Full basket!" : ""}</span>
    ...
    els.nextBtn.textContent = state.index + 1 < Levels.LEVELS.length ? "Next path →" : "Level select";
    ...
    for (const s of Art.SKINS) {
      ...
      if (wasLocked && skinUnlocked(s)) {
        SFX.unlock();
        toast(`${s.name} unlocked`, "Change cloak in Level select");
      }
    }
    if (fullBasket) toast("Full basket!", "Grandmother will be delighted");
  }
  ...
  canvas.addEventListener("contextlost", (e) => {
    e.preventDefault();
    console.warn("[Crimson Path] canvas context lost");
  });
    ...
    for (const e of ents) {
      ...
      switch (e.type) {
        ...
        case "wolf": {
          ...
          const wob = Math.sin(e.walk * 8) * 1.2;
          const dir = e.dir || -1;
          // body
          const bg = ctx.createLinearGradient(0, -28, 0, -2);
          bg.addColorStop(0, "#6a5a50");
          bg.addColorStop(1, "#3a3028");
          ctx.fillStyle = bg;
          ctx.beginPath();
          ctx.ellipse(0, -12 + wob * 0.3, 16, 11, 0, 0, Math.PI * 2);
          ctx.fill();
          // head
          ctx.fillStyle = "#5a4a40";
          ctx.beginPath();
          ctx.ellipse(dir * 12, -16 + wob * 0.2, 9, 8, 0, 0, Math.PI * 2);
          ctx.fill();
          // snout
          ctx.fillStyle = "#4a3a32";
          ctx.beginPath();
          ctx.ellipse(dir * 18, -13, 5, 3.5, 0, 0, Math.PI * 2);
          ctx.fill();
          ctx.fillStyle = "#1a1410";
          ctx.beginPath();
          ctx.arc(dir * 20, -13, 1.6, 0, Math.PI * 2);
          ctx.fill();
          // ear
          ctx.fillStyle = "#4a3a32";
          ctx.beginPath();
          ctx.moveTo(dir * 8, -24);
          ctx.lineTo(dir * 12, -32);
          ctx.lineTo(dir * 16, -23);
          ctx.closePath();
          ctx.fill();
          // eye
          ctx.fillStyle = "#f0c040";
          ctx.beginPath();
          ctx.ellipse(dir * 14, -17, 2.2, 2.6, 0, 0, Math.PI * 2);
          ctx.fill();
          ctx.fillStyle = "#1a1008";
          ctx.beginPath();
          ctx.arc(dir * 14.5, -17, 1.1, 0, Math.PI * 2);
          ctx.fill();
          // tail
          ...
          ctx.lineCap = "round";
          ctx.beginPath();
          ctx.moveTo(-dir * 14, -10);
          ctx.quadraticCurveTo(-dir * 22, -18 + Math.sin(e.walk * 6) * 3, -dir * 26, -8);
          ctx.stroke();
          ctx.restore();
          break;
        }
        ...
        case "exit": {
          ...
          ctx.fill();
          const wall = ctx.createLinearGradient(-28, -40, 28, 0);
          wall.addColorStop(0, "#8a6040");
          wall.addColorStop(0.5, "#c48a58");
          wall.addColorStop(1, "#7a5030");
          ctx.fillStyle = wall;
          Art.roundRect(ctx, -26, -36, 52, 38, 4);
          ctx.fill();
          // roof
          ctx.fillStyle = lit ? "#a03028" : "#702820";
          ctx.beginPath();
          ctx.moveTo(-32, -34);
          ctx.lineTo(0, -62);
          ctx.lineTo(32, -34);
          ...
          ctx.fill();
          // door
          ctx.fillStyle = lit ? "#5a3820" : "#3a2818";
          Art.roundRect(ctx, -8, -28, 16, 28, 3);
          ctx.fill();
          // window glow
          ctx.fillStyle = lit ? "#ffe08a" : "#4a4030";
          Art.roundRect(ctx, 10, -28, 10, 10, 2);
          ctx.fill();
          if (lit) {
            ctx.globalCompositeOperation = "lighter";
            const g = ctx.createRadialGradient(0, -20, 0, 0, -20, 50);
            ...
            ctx.fill();
            Art.drawEmber(ctx, 0, -48, 5, t, "rgba(255,246,214,0.95)", "rgba(255,170,70,0.5)");
          }
          ctx.restore();
          break;
        }
  ...
  function crash(err) {
    ...
    if (!box) return;
    box.hidden = false;
    box.innerHTML = `<strong>The path went dark.</strong><span>${String(
      (err && err.message) || err,
  ...
  function hideCards() {
    ...
    for (const el of [els.title, els.map, els.pause, els.win, els.story]) {
  ...
  function showTitle() {
    ...
    els.titleBest.textContent = cleared
      ? `${cleared}/${Levels.LEVELS.length} paths cleared · ${totalSeeds()} crumbs · ${totalPetals()} wildflowers`
      : "Ten baskets. One wolf. Reach grandmother.";
    if (!level) {
  ...
  function showMap() {
    ...
    els.mapTotals.textContent = `${totalSeeds()} 🥧 · ${totalPetals()}/10 🌼 · ${clearedCount()}/${Levels.LEVELS.length} cleared`;
  ...
  if (els.storyContinue) {
    els.storyContinue.addEventListener("click", () => {
      audioReady();
      SFX.ui();
      continueStory();
    });
