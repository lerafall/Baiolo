/* Foxfire Duel — painted art layer.
 *
 * Shares its look with Foxfire Hollow: same palettes, same fox, same baked
 * parallax. The tile-level baker is replaced by a single floating arena.
 *
...
(() => {
  ...
  /**
   * One floating island, baked whole. The duel shrinks the platform by drawing
   * a narrower slice of this image, so the art has to be uniform across its
   * width — no landmarks that would slide as the edges crumble.
   */
  function bakeArena(widthPx, biome) {
    const p = PALETTES[biome];
    const SURFACE = 44; // grass blades need headroom above the rock
    const H = 340;
    const c = surface(widthPx, H);
    const ctx = c.getContext("2d");
    const rnd = mulberry32(0xa2e17 ^ widthPx);
    const bodyTop = SURFACE;
    const bodyBottom = SURFACE + 96;

    // --- silhouette: slab on top, roots tapering into the dark below
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(0, bodyTop);
    ctx.lineTo(widthPx, bodyTop);
    ctx.lineTo(widthPx, bodyBottom);
    for (let x = widthPx; x >= 0; x -= 24) {
      const t = x / widthPx;
      const droop = Math.sin(t * Math.PI) * 66 + Math.sin(x * 0.05) * 9;
      ctx.lineTo(x, bodyBottom + droop);
    }
    ctx.lineTo(0, bodyBottom);
    ctx.closePath();
    ctx.clip();

    const g = ctx.createLinearGradient(0, bodyTop, 0, bodyBottom + 90);
    g.addColorStop(0, p.rockLight);
    g.addColorStop(0.34, p.rockMid);
    g.addColorStop(1, p.rockDeep);
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, widthPx, H);

    // strata and speckle so the rock does not read as a flat bar
    for (let i = 0; i < 7; i++) {
      const y = bodyTop + 14 + i * 15;
      ctx.strokeStyle = i % 2 ? "rgba(0,0,0,0.13)" : "rgba(255,255,255,0.06)";
      ctx.lineWidth = 1.5 + rnd() * 2;
      ctx.beginPath();
      for (let x = -10; x <= widthPx + 10; x += 26) {
        ctx.lineTo(x, y + Math.sin(x * 0.02 + i) * 5 + rnd() * 3);
      }
      ctx.stroke();
    }
    for (let i = 0; i < widthPx / 3; i++) {
      const x = rnd() * widthPx;
      const y = bodyTop + rnd() * (bodyBottom - bodyTop + 80);
      ctx.fillStyle = rnd() > 0.5 ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.12)";
      ctx.beginPath();
      ctx.arc(x, y, 0.7 + rnd() * 2.3, 0, Math.PI * 2);
      ctx.fill();
    }
    for (let i = 0; i < widthPx / 90; i++) {
      const cx = rnd() * widthPx;
      const cy = bodyTop + 34 + rnd() * 60;
      const rx = 22 + rnd() * 26;
      const bg = ctx.createLinearGradient(cx, cy - rx, cx, cy + rx);
      bg.addColorStop(0, hexA(p.rockLight, 0.2));
      bg.addColorStop(1, "rgba(0,0,0,0.22)");
      ctx.fillStyle = bg;
      ctx.beginPath();
      ctx.ellipse(cx, cy, rx, rx * 0.62, (rnd() - 0.5) * 0.8, 0, Math.PI * 2);
      ctx.fill();
    }
    // light from above, dark towards the roots
    const shade = ctx.createLinearGradient(0, bodyBottom - 10, 0, bodyBottom + 80);
    shade.addColorStop(0, "rgba(0,0,0,0)");
    shade.addColorStop(1, "rgba(0,0,0,0.55)");
    ctx.fillStyle = shade;
    ctx.fillRect(0, bodyBottom - 10, widthPx, 100);
    ctx.restore();

    // --- mossy top, reusing the Hollow's cap so both games match
    for (let x = 0; x < widthPx; x += TILE) {
      drawCap(ctx, biome, p, x, bodyTop, rnd, x > 0);
    }
    ctx.fillStyle = "rgba(255,255,255,0.22)";
    ctx.fillRect(0, bodyTop - 1, widthPx, 2);

    // --- roots and vines trailing off the underside
    for (let i = 0; i < widthPx / 26; i++) {
      const x = rnd() * widthPx;
      const t = x / widthPx;
      const from = bodyBottom + Math.sin(t * Math.PI) * 60;
      const len = 14 + rnd() * 46;
      ctx.strokeStyle = biome === "cavern" ? "rgba(150,90,170,0.45)" : "rgba(74,56,40,0.6)";
      ctx.lineWidth = 1.4 + rnd() * 2;
      ctx.beginPath();
      ctx.moveTo(x, from - 4);
      ctx.quadraticCurveTo(x + (rnd() - 0.5) * 12, from + len * 0.6, x + (rnd() - 0.5) * 16, from + len);
      ctx.stroke();
    }

    return { canvas: c, surfaceY: bodyTop };
  }
